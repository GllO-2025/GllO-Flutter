...
class AppProgressBar extends StatelessWidget {
  AppProgressBar({required this.value, required this.max, super.key})
    : assert(max > 0 && value >= 0 && max >= value) {
    if (max <= 0 || value < 0 || max < value) {
      throw ArgumentError('Invalid progress values: value=$value, max=$max');
    }
  }
  final double value;
  final double max;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final double indicatorWidth = m.max(
          m.min(value / max, 1) * trackWidth,
          4,
        );
